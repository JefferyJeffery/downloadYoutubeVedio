...
NSString * const kInfoURLEncodedFmtStreamMap = @"url_encoded_fmt_stream_map";
NSString * const kYoutubeInfoStatus = @"status";

NSString * const vYoutubeStatusOK = @"ok";
NSString * const vYoutubeStatusFail = @"fail";

//ErrorReason
NSString * const kYoutubeErrorReason = @"reason";

//Video
NSString * const kYoutubeVideoHD720 = @"hd720";
NSString * const kYoutubeVideoMedium = @"medium";
NSString * const kYoutubeVideoSmall = @"small";

//live
NSString * const kYoutubeVideoLive = @"live";
...
+ (NSDictionary *)h264videosWithYoutubeID:(NSString *)youtubeID {
    if (youtubeID) {
        ...
        if (!error) {
            ...
            if (parts) {
                NSString *status = [parts valueForKeyPath:kYoutubeInfoStatus][0];

                NSMutableDictionary *videoDictionary = [NSMutableDictionary dictionary];
                [videoDictionary setObject:status forKey:kYoutubeInfoStatus];
                
                if ([status isEqualToString:@"fail"]) {
                    NSString *errorReason = [parts valueForKeyPath:kYoutubeErrorReason][0];
                    [videoDictionary setObject:errorReason forKey:kYoutubeErrorReason];
                    return videoDictionary;

                } else {
                    NSString *fmtStreamMapString = [[parts objectForKey:kInfoURLEncodedFmtStreamMap] objectAtIndex:0];
                    ...
                    // Check for live data
                        ...
                        NSString *hlsvp = [parts valueForKeyPath:@"hlsvp"][0];
                        [videoDictionary setObject:hlsvp forKey:kYoutubeVideoLive];
                        return videoDictionary;
                    }
                }
            }
